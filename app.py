...
# ------------------------------------------------------------------------------
# ADMIN: VIEW & DOWNLOAD UPLOADED FILES FROM STORAGE
# ------------------------------------------------------------------------------
st.markdown("---")
st.header("📁 Server Storage Explorer (Uploaded CVs & JDs)")

col_store1, col_store2 = st.columns(2)

with col_store1:
    st.subheader("📄 Stored CVs")
    if os.path.exists(STORAGE_CVS):
        cv_files = os.listdir(STORAGE_CVS)
        if cv_files:
            for cv_f in cv_files:
                f_path = os.path.join(STORAGE_CVS, cv_f)
                with open(f_path, "rb") as file_data:
                    st.download_button(
                        label=f"Download {cv_f}",
                        data=file_data,
                        file_name=cv_f,
                        key=f"dl_cv_{cv_f}"
                    )
        else:
            st.info("No CV files found in storage.")

with col_store2:
    st.subheader("📑 Stored JDs")
    if os.path.exists(STORAGE_JDS):
        jd_files = os.listdir(STORAGE_JDS)
        if jd_files:
            for jd_f in jd_files:
                f_path = os.path.join(STORAGE_JDS, jd_f)
                with open(f_path, "rb") as file_data:
                    st.download_button(
                        label=f"Download {jd_f}",
                        data=file_data,
                        file_name=jd_f,
                        key=f"dl_jd_{jd_f}"
                    )
        else:
            st.info("No JD files found in storage.")